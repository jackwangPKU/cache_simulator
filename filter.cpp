/* 
 * This program filters the traces given slice_no and set_no based on the file "cal_set_slice.cpp".
 * Usage: g++ filter.cpp -o filter
 *        ./filter [benchmark1] [benchmark2]
 * Input: follow the hints
 * Output: the traces of a certain set of the slice, saved as [benchmark1]_[benchmark2]_[slice_no]_[set_no].out
 * Author: Jack Wang
 * Date: 2019.10.21
 */
...
unsigned long long addr1, addr2, chosen_set_no;
int chosen_slice_no;
...
int main(int argc, char *argv[])
{
    ...
    while(fgets(tmp1, 99, file1) != NULL && fgets(tmp2, 99, file2) != NULL)   // end with either file finished
    {
        ...
        if((slice1 == chosen_slice_no) && (set_no1 == chosen_set_no))
            fprintf(outfile, "%llu\n", addr1);
        ...
        if((slice2 == chosen_slice_no) && (set_no2 == chosen_set_no))
            fprintf(outfile, "%llu\n", addr2);
